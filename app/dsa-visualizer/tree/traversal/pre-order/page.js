...
const BinaryTreePreOrder = () => {
  ...
  // Generate pre-order traversal steps
  const generateSteps = (tree) => {
    ...
    steps.push({
      ...
      visited: [],
      result: [],
      description: 'Starting pre-order traversal: ROOT → LEFT → RIGHT. We process the node first, then recursively visit children.'
    })
    ...
    const preorderTraversal = (node, path = '') => {
      ...
      steps.push({
        ...
        result: [...result],
        description: `Visiting node ${node.value}${path}. In pre-order, we process the ROOT first.`
      })
      ...
      steps.push({
        ...
        visited: [...result.map(n => n.id)],
        result: [...result],
        description: `Processed node ${node.value}. Added to result: [${result.map(n => n.value).join(', ')}]`
      })

      // Visit left subtree (LEFT)
      if (node.left) {
        steps.push({
          type: 'go_left',
          current: node.id,
          visited: [...result.map(n => n.id)],
          result: [...result],
          description: `Now traverse LEFT subtree of node ${node.value}.`
        })
    ...
    steps.push({
      ...
      visited: result.map(n => n.id),
      result: result,
      description: `Pre-order traversal complete! Result: [${result.map(n => n.value).join(', ')}] - Root was processed first!`
    })
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Root → Left → Right traversal pattern</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
              ...
              {/* Pre-order Pattern */}
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                <h3 className="text-white font-medium mb-2">Pre-Order Pattern:</h3>
              ...
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                ...
                <p className="text-gray-300">
                  {steps[currentStep]?.description || 'Click play to start the pre-order traversal'}
                </p>
              </div>
              ...
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                ...
                <div className="text-gray-300 text-sm space-y-1">
                  ...
                  <p><strong>Use Case:</strong> Tree copying, prefix expression evaluation</p>
                  <p><strong>Pattern:</strong> Process current node before its children</p>
                </div>
              </div>
            </div>
          </div>
...
export default BinaryTreePreOrder
