...
const BinaryTreePostOrder = () => {
  ...
  // Generate post-order traversal steps
  const generateSteps = (tree) => {
    ...
    steps.push({
      ...
      visited: [],
      result: [],
      description: 'Starting post-order traversal: LEFT → RIGHT → ROOT. We process children before the parent.'
    })

    const postorderTraversal = (node, path = '') => {
      if (!node) return
      ...
      steps.push({
        ...
        result: [...result],
        description: `Visiting node ${node.value}${path}. In post-order, we process children first.`
      })

      // Visit left subtree first (LEFT)
      if (node.left) {
        steps.push({
          type: 'go_left',
          current: node.id,
          visited: [...result.map(n => n.id)],
          result: [...result],
          description: `First traverse LEFT subtree of node ${node.value}.`
        })
        ...
        steps.push({
          ...
          visited: [...result.map(n => n.id)],
          result: [...result],
          description: `No left child for node ${node.value}. Move to right subtree.`
        })
      }

      // Visit right subtree (RIGHT)
      if (node.right) {
        steps.push({
          type: 'go_right',
          current: node.id,
        ...
        steps.push({
          ...
          visited: [...result.map(n => n.id)],
          result: [...result],
          description: `No right child for node ${node.value}. Now process this node.`
        })
      }
      ...
      steps.push({
        ...
        visited: [...result.map(n => n.id)],
        result: [...result],
        description: `Finally process node ${node.value} after both children. Added to result: [${result.map(n => n.value).join(', ')}]`
      })
    }
    ...
    steps.push({
      ...
      visited: result.map(n => n.id),
      result: result,
      description: `Post-order traversal complete! Result: [${result.map(n => n.value).join(', ')}] - Children processed before parents!`
    })
  ...
  const renderNode = (node, x, y, level = 0) => {
    ...
    return (
      <g key={node.id}>
        ...
        {/* Show processing order for visited nodes */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Left → Right → Root traversal pattern</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
              ...
              {/* Post-order Pattern */}
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                <h3 className="text-white font-medium mb-2">Post-Order Pattern:</h3>
              ...
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                ...
                <p className="text-gray-300">
                  {steps[currentStep]?.description || 'Click play to start the post-order traversal'}
                </p>
              </div>
              ...
              <div className="mt-6 bg-gray-700 rounded-lg p-4">
                ...
                <div className="text-gray-300 text-sm space-y-1">
                  ...
                  <p><strong>Use Case:</strong> Tree deletion, calculating directory sizes, postfix expressions</p>
                  <p><strong>Pattern:</strong> Process current node after both children</p>
                </div>
              </div>
            </div>
          </div>
...
export default BinaryTreePostOrder
