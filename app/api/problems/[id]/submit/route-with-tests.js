      ...
      for (let i = 0; i < problem.testCases.length; i++) {
        ...
        try {
          ...
          const userFunction = new Function(code + `
            ...
            const functionMatch = code.match(/(?:function\\s+|var\\s+|let\\s+|const\\s+)([a-zA-Z_$][a-zA-Z0-9_$]*)\\s*[=\\(]/);
            if (!functionMatch) throw new Error('No function found');
            const functionName = functionMatch[1];
            
            // Call the function with test inputs
            if (Array.isArray(parsedInput)) {
              return ${getFunctionName(code)}(...parsedInput);
            } else {
              return ${getFunctionName(code)}(parsedInput);
            }
          `);
