...
const TopicSelectionModal = ({ isOpen, onClose, onStartInterview, interviewType }) => {
  ...
  const topicOptions = {
    'technical-coding': [
      ...
      { id: 'algorithms', name: 'Algorithms', icon: FaBrain, color: 'theme-accent' }
    ],
    'system-design': [
    ...
    'behavioral': [
      { id: 'leadership', name: 'Leadership', icon: FaUsers, color: 'theme-accent' },
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="theme-surface border border theme-border rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all duration-300 hover:scale-105 ${
                    isSelected
                      ...
                      : 'border-gray-600 theme-surface-elevated/30 text-gray-300 hover:border-purple-400'
                  }`}
                >
                  <IconComponent className={`text-2xl mx-auto mb-2 ${isSelected ? 'theme-accent' : topic.color}`} />
        ...
        {/* Interview Summary */}
        <div className="theme-surface-elevated/30 rounded-lg p-4 mb-6">
          <h4 className="text-white font-semibold mb-2">Interview Summary</h4>
          <div className="text-sm text-gray-300 space-y-1">
            <p><span className="theme-accent">Topics:</span> {selectedTopics.length > 0 ? selectedTopics.join(', ') : 'None selected'}</p>
            <p><span className="theme-accent">Difficulty:</span> {difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}</p>
            <p><span className="theme-accent">Questions:</span> {questionCount}</p>
            <p><span className="theme-accent">Estimated Time:</span> {questionCount * (difficulty === 'easy' ? 15 : difficulty === 'medium' ? 25 : 35)} minutes</p>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex-1 bg-blue-600 hover:bg-blue-700 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Start AI Interview
          </button>
        </div>
