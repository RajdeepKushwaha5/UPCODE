...
export default function ComprehensiveContestManagement() {
  ...
  useEffect(() => {
    ...
    // Set up real-time updates every 60 seconds for contests
  ...
  const fetchContests = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Unauthorized access - please login as admin');
        alert('Please login as an admin to access contest management');
        setContests([]);
      } else if (response.status === 403) {
        console.error('Admin access required');
        alert('Admin privileges required to access contest management');
        setContests([]);
      } else {
        const errorData = await response.json();
        console.error('Failed to fetch contests:', errorData);
        setContests([]);
      }
    } catch (error) {
  ...
  const fetchProblems = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch problems, using fallback data');
        // Fallback mock problems if API fails
        const mockProblems = [
      ...
      // Fallback mock problems on error
      const mockProblems = [
        { _id: "p1", title: "Two Sum", difficulty: "Easy" },
        { _id: "p2", title: "Valid Parentheses", difficulty: "Easy" },
        { _id: "p3", title: "Longest Palindrome", difficulty: "Medium" },
  ...
  const handleCreateContest = async (e) => {
    ...
    try {
      const contestData = {
        ...
        start: formData.startTime, // Map to API expected field
        end: formData.endTime,     // Map to API expected field
        duration: parseInt(formData.duration),
        type: formData.type,
        difficulty: formData.difficulty || 'Medium',
        prize: formData.prizes.length > 0 ? formData.prizes[0].prize : 'TBD',
        rating: '1200-2000', // Default rating
        problems: formData.problems || [],
        link: '', // Will be generated by API
        source: 'internal'
      };

      console.log('Creating contest with data:', contestData); // Debug log
